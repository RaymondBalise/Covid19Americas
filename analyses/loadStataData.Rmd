...
```{r}
analysis <- read_stata("../data/Mexico COVID-19 April 15.dta") %>% 
  ...
  mutate(State_Name = stri_encode(State_Name, "", "UTF-8")) %>% # accents
  mutate(`State Name` = case_when(State_Name == "Michoac�n" ~ "Michoacan",
                                State_Name == "Nuevo Le�n" ~ "Nuevo Leon",
                                State_Name == "Quer�taro" ~"Queretaro",
                                State_Name == "San Luis Potos�" ~ "San Luis Potosi",
                                State_Name == "Yucat�n" ~ "Yucatan",
                                TRUE ~ State_Name))
